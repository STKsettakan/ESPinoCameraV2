...
esp_err_t camera_probe(const camera_config_t* config, camera_model_t* out_camera_model)
{
    ...
    camDebugln("Enabling XCLK output");
    camera_enable_out_clock(config);
	delay(500);
    camDebugln("Initializing SSCB");
    SCCB_Init(config->pin_sscb_sda, config->pin_sscb_scl);

    camDebugln("Resetting camera");
   ...
   camDebugln("Searching for camera address");
    /* Probe the sensor */
    delay(10);
    ...
    sprintf(str,"Detected camera at address=0x%02x", slv_addr);
	camDebugln(str);
    ...
    delay(10);
     sprintf(str,"Camera PID=0x%02x VER=0x%02x MIDL=0x%02x MIDH=0x%02x",
    ...
    switch (id->PID) {
#if CONFIG_OV2640_SUPPORT
        case OV2640_PID:
			camDebugln("CAMERA Model OV2640");
...
#if CONFIG_OV7725_SUPPORT
        case OV7725_PID:
			camDebugln("CAMERA Model OV7725");
            *out_camera_model = CAMERA_OV7725;
            ov7725_init(&s_state->sensor);
            break;
#endif
        default:
			camDebugln("CAMERA Model UNKNOWN");
            id->PID = 0;
     ...
     camDebugln("Doing SW reset of sensor");
...
esp_err_t camera_init(const camera_config_t* config)
{
    ...
    if (!s_state) {
		camDebugln("ESP_ERR : INVALID STATE");
        return ESP_ERR_INVALID_STATE;
    }
    if (s_state->sensor.id.PID == 0) {
		camDebugln("ESP_ERR : CAMERA_NOT_SUPPORTED");
        return ESP_ERR_CAMERA_NOT_SUPPORTED;
    }
    ...
    sprintf(str,"Setting frame size to %dx%d", s_state->width, s_state->height);
	camDebugln(str);
    if (s_state->sensor.set_framesize(&s_state->sensor, frame_size) != 0) {
        camDebugln("Failed to set frame size");
    ...
    if (pix_format == PIXFORMAT_GRAYSCALE) {
        if (s_state->sensor.id.PID != OV7725_PID) {
             camDebugln("Grayscale format is only supported for ov7225");
        ...
        if (s_state->sensor.id.PID != OV7725_PID) {
            camDebugln("RGB565 format is only supported for ov7225");
            err = ESP_ERR_NOT_SUPPORTED;
            goto fail;
        }
        ...
        if (s_state->sensor.id.PID != OV2640_PID) {
            camDebugln( "JPEG format is only supported for ov2640");
            err = ESP_ERR_NOT_SUPPORTED;
            goto fail;
        }
        ...
        s_state->fb_bytes_per_pixel = 2;
         ...
         camDebugln( "Requested format is not supported");
        err = ESP_ERR_NOT_SUPPORTED;
        goto fail;
    }

     sprintf(str,"in_bpp: %d, fb_bpp: %d, fb_size: %d, mode: %d, width: %d height: %d",
            s_state->in_bytes_per_pixel, s_state->fb_bytes_per_pixel,
            s_state->fb_size, s_state->sampling_mode,
            s_state->width, s_state->height);
	camDebugln(str);
	sprintf(str,"Allocating frame buffer (%d bytes)", s_state->fb_size);
	camDebugln(str);
	
    s_state->fb = (uint8_t*) calloc(s_state->fb_size, 1);
    if (s_state->fb == NULL) {
        camDebugln("Failed to allocate frame buffer");
        err = ESP_ERR_NO_MEM;
        goto fail;
    }

    camDebugln("Initializing I2S and DMA");
    i2s_init();
    err = dma_desc_init();
    if (err != ESP_OK) {
         camDebugln("Failed to initialize I2S and DMA");
        goto fail;
    }
    ...
    if (s_state->data_ready == NULL || s_state->frame_ready == NULL) {
         camDebugln("Failed to create semaphores");
        err = ESP_ERR_NO_MEM;
        goto fail;
    }
    if (!xTaskCreatePinnedToCore(&dma_filter_task, "dma_filter", 4096, NULL, 10, &s_state->dma_filter_task, 1)) {
       camDebugln("Failed to create DMA filter task");
        err = ESP_ERR_NO_MEM;
        goto fail;
    }

    camDebugln("Initializing GPIO interrupts");
    vsync_intr_disable();
    err = gpio_isr_handler_add(s_state->config.pin_vsync, &gpio_isr, NULL);
    if (err != ESP_OK) {
       sprintf(str,"gpio_isr_handler_add failed (%x)", err);
	    camDebugln(str);
        goto fail;
    }
